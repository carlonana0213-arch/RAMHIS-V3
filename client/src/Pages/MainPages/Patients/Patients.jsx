...
export default function Patients() {
  ...
  */
  ...
  const fetchCurrentMission = useCallback(async () => {
    ...
    */
    ...
    if (!navigator.onLine) {
      const cachedMission =
        await getOfflineMeta("currentMission");

      setOngoingEvent(
        cachedMission?.value || null,
      );
  ...
  */
  ...
  useEffect(() => {
    ...
    cachePatientQueueForOffline().catch(
      (error) => {
        console.error(
          "Failed to prepare Patient data for offline use:",
          error,
        );
      },
    );
